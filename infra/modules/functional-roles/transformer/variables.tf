...
variable "ingestion_schema" {
  description = "Fully qualified name of the ingestion/landing schema, read by dbt sources."
  type        = string
}

variable "transformation_schemas" {
  description = "Fully qualified names of the transformation schemas (STAGING/INTERMEDIATE/MARTS)."
  type        = list(string)
}
